...
def shift_corruption(
    im: ImageType, offset_mag: int, width: int, coverage: float
) -> ImageType:
    ...
    """
    corrupted = im
    line_count = int(im.size[1] / width * coverage)
    for start_line in random.choices(range(0, im.size[1], width), k=line_count):
        offset = random.randint(-offset_mag, offset_mag)
        for ypos in range(start_line, start_line + width):
